...
if "context" not in streamlit.session_state:
    streamlit.session_state["context"] = []
...
if prompt:
    with streamlit.spinner("Generating Response..."):
        generated_response = run_llm(query=prompt, context=streamlit.session_state["context"])
        sources = set(
        ...
        streamlit.session_state["context"].append((prompt, generated_response["answer"]))
